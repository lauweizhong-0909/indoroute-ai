...
export default function ProfitPage() {
  ...
  const alertProfits = profits.filter((profit) => profit.alert);
  const worstProfit = alertProfits[0] ?? null;
  ...
  const shippingCost = selectedProfit?.alert ? 1.92 : 12.0;
  const dutyCost = selectedProfit?.alert ? 0 : 9.5;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Card className="bg-slate-900 text-white border-0 shadow-md">
          <CardContent className="p-4 flex items-center gap-4">
            <div>
              <p className="text-xs text-slate-400">Live Rate (MYR/IDR)</p>
          ...
          <CardContent className="p-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-2">
              ...
              <h2 className="text-2xl font-semibold tracking-tight">
                {worstProfit.sku_id} is currently losing {formatMYR(Math.abs(worstProfit.net_profit_myr))} per unit
              </h2>
              ...
              <TableBody>
                {skus.map((sku) => {
                  ...
                  const marginColor = !profit ? 'text-slate-500' :
                    profit.margin_pct < 0.05 ? 'text-red-600 font-bold' :
                    profit.margin_pct < 0.15 ? 'text-yellow-600 font-bold' : 'text-green-600 font-bold';
                  
                  return (
                    <TableRow 
                      key={sku.sku_id} 
                      className={`cursor-pointer ${profit?.alert ? 'bg-red-500/10 hover:bg-red-500/20 border-red-500/20' : ''}`}
      ...
      <Sheet open={!!selectedProfit} onOpenChange={(open) => !open && setSelectedProfit(null)}>
        <SheetContent className="overflow-y-auto px-8">
          ...
          <div className="space-y-6">
            <div className={`rounded-lg border p-5 ${selectedProfit?.alert ? 'bg-red-500/10 border-red-500/20' : 'bg-blue-500/10 border-blue-500/20'}`}>
              <div className="flex items-start gap-3">
                <Info className={`w-5 h-5 mt-0.5 shrink-0 ${selectedProfit?.alert ? 'text-red-500' : 'text-blue-400'}`} />
                <div className="space-y-2">
                  <p className={`text-xs font-semibold uppercase tracking-wide ${selectedProfit?.alert ? 'text-red-500' : 'text-blue-300'}`}>Result</p>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-lg border border-border bg-muted/40 p-4">
                ...
                <p className="mt-2 text-lg font-semibold">{selectedSkuRecord ? formatMYR(selectedSkuRecord.cost_myr + dutyCost + shippingCost) : "-"}</p>
                <p className="text-sm text-muted-foreground">Product, duty, shipping.</p>
              </div>
              <div className={`rounded-lg border p-4 ${selectedProfit?.alert ? 'border-red-500/20 bg-red-500/10' : 'border-green-500/20 bg-green-500/10'}`}>
                <p className={`text-xs font-semibold uppercase tracking-wide ${selectedProfit?.alert ? 'text-red-500' : 'text-green-600'}`}>Net result</p>
            ...
            <div className="bg-muted p-4 rounded-lg border border-border">
              ...
              <div className="font-mono text-sm space-y-2 text-slate-700">
                ...
                <div className="flex justify-between text-red-600"><span>- Import Duty / VAT:</span> <span>{dutyCost.toFixed(2)}</span></div>
                <div className="flex justify-between text-red-600 border-b border-slate-300 shadow-sm pb-2"><span>- Shipping / Fulfillment:</span> <span>{shippingCost.toFixed(2)}</span></div>
